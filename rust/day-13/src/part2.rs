...
pub fn main(input: &str) -> String { 
    ...
    l.push(NestedList::List(vec![NestedList::Value(2)]));
    l.push(NestedList::List(vec![NestedList::Value(6)]));
    l.sort();
    let a = l.iter().position(|r| r == &NestedList::List(vec![NestedList::Value(2)])).unwrap();
    let b = l.iter().position(|r| r == &NestedList::List(vec![NestedList::Value(6)])).unwrap();
